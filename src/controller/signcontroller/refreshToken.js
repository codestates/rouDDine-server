...
module.exports = async (req, res) => {
  ...
  let userInfo = await users.findOne({where: {userId: decode.userId, email: decode.email}});
  
  let data = {...userInfo.dataValues};
  delete data.password; //password만 정보 삭제
  const accessToken = await jwt.sign(data, process.env.ACCESS_SECRET, {expiresIn : '1h'}) 
  res.status(200).send({data: {userInfo: data, accessToken: accessToken}, message: 'ok'}); // 이 부분 확인 필요.
