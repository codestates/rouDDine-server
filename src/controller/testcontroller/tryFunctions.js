...
module.exports = {
  ...
  create_exercise: async (req, res) => {
    if (!req.cookies.accessToken || req.cookies.accessToken === "") {
      ...
      const routineinfo = await routine.findOne({
      ...
      if (routineinfo) {
        ...
        const remainexercises = await exercise.findAll({
          where: { routine_id: newCard.routine_id },
        });
        let workout = [];
    ...
    if (!req.query.workoutid) {
      ...
      if (card) {
        ...
        if (routineinfo) {
          ...
          const remainexercises = await exercise.findAll({
            where: { routine_id: card.routine_id },
          });
          let workout = [];
          for (let i = 0; i < remainexercises.length; i++) {
            workout.push(remainexercises[i]);
  ...
  update_exercise: async (req, res) => {
    if (!req.body.workoutid) {
      ...
      if (ex_card) {
        ...
        const remainexercises = await exercise.findAll({
          where: { routine_id: ex_card.routine_id },
        });
    ...
    if (!req.query.routine_id) {
      ...
      if (finduser) {
        ...
        routineCard = await routine.findAll({
          //로그인 한 경우 자기 루틴과 기본루틴 검색
          where: {
            [Op.or]: [{ userid: data.id }, { default: true }],
          },
        });
      } else {
        //로그인 안한경우 기본 루틴만 검색
    ...
    if (!req.body.routine_id) {
      ...
      if (routinecard) {
        if (routinecard.default === true) {
            ...
            if (req.body.exercise_array.length !== 0) {
              ...
              for (let i = 0; i < req.body.exercise_array.length; i++) {
                let ex = await exercise.findOne({
                  where: { id: req.body.exercise_array[i].id },
                });
                if (ex.set_number && (ex.set_time_min || ex.set_time_sec)) {
                  time +=
                    (ex.set_time_min * 60 + ex.set_time_sec) * ex.set_number; //운동 총 시간 계산
                }
          ...
          const excards = await exercise.findAll({
            where: { routine_id: routinecard.id },
          });
